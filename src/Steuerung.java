...
public class Steuerung implements Runnable{
	...
	public Steuerung() {
		dasSpielfeld = new Spielfeld(0.05,108*8);//Wird später ein Feld, wenn Mars und Erde kommen
		spielfeldErstellen();
	...
	private void bodenBeruehrung() {
		...
		for(int i=0; i < dasSpielfeld.getAnzahlPunkte()-1; i++) {
			...
			if((x1 <= xL && xR <= x2)||(x1 <= xR && xL <= x2)) {
				...
				double xS = (cU-c)/(m-mU);
				//Ist die Schnittstelle im x Intervall der Gerade des Spielfelds und Unterseite?
